...
fn test_full_game_flow() {
    ...
    let (_, player_result) = apply_combat_damage(&player_card, &mut opponent_copy);
    let (_, opponent_result) = apply_combat_damage(&opponent_card, &mut player_copy);
    ...
    // At least one hand should have fewer cards now (or same if no cards died)
    assert!(player_dead + opponent_dead >= 0);
...
fn test_card_damage_application() {
    ...
    assert!(dmg1 < card1.hp, "Damage should be less than attack value when dominant");
